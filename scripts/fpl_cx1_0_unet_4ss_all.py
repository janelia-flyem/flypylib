...
generator  = fplobjdetect.gen_volume(
    train_data, network.rf_size, batch_size*n_gpu, 0.5)
...
out_dir = '/groups/flyem/home/yangh3/Research/results/unet-like/gpus-4-epochs-20/trained-all/net-24-6'

for ee in range(1):
    network.train(generator, 1000, 20)
    ...
    filename = '%s/pr-agg.pdf' % out_dir
    plt.savefig(filename, bbox_inches='tight')
    plt.show()

    filename = '%s/results.pickle' % out_dir
    with open(filename, 'wb') as f:
        pickle.dump([mm_train_agg, mm_test_agg, mm_train, mm_test], f)

filename = '%s/net' % out_dir
